...
def main(voice, original_text, model_name, text_file, folder, global_index, index, retry, threshold):
    ...
    for i in tqdm(range((len(text) // global_index) * index, (len(text) // global_index) * (index + 1), 1)):
