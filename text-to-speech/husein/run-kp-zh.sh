...
for i in {0..3}; do
  ...
  python3 generate_zh.py \
    ...
    --global_index 4 \
    --index $i --threshold -10 || echo 'Error encountered! Press Ctrl+D to exit.'; exec bash"
done
...
for i in {0..3}; do
  screen -S "run_kp-chinese-texts-part3_$i" -X quit 2>/dev/null
  screen -dmS "run_kp-chinese-texts-part3_$i" bash -c "cd /workspace && \
  CUDA_VISIBLE_DEVICES=2 \
  python3 generate_zh.py \
    --voice \"kp-rtm-enhanced-v2.wav\" \
    --original_text \"Selalunya kalau kita pergi berjumpa media kan, kalau kita pergi buat produk launch ke dan sebagainya, selalunya, dia siapkan barang itu dulu, atau apa yang kita nak jual kepada dunia,\" \
    --text_file \"chinese-texts-part3.json\" \
    --folder \"kp-chinese-texts-part3\" \
    --global_index 4 \
    --index $i --threshold -10"
done

for i in {0..3}; do
  screen -S "run_kp-chinese-texts-part4_$i" -X quit 2>/dev/null
  screen -dmS "run_kp-chinese-texts-part4_$i" bash -c "cd /workspace && \
  CUDA_VISIBLE_DEVICES=3 \
  python3 generate_zh.py \
    --voice \"kp-rtm-enhanced-v2.wav\" \
    --original_text \"Selalunya kalau kita pergi berjumpa media kan, kalau kita pergi buat produk launch ke dan sebagainya, selalunya, dia siapkan barang itu dulu, atau apa yang kita nak jual kepada dunia,\" \
    --text_file \"chinese-texts-part4.json\" \
    --folder \"kp-chinese-texts-part4\" \
    --global_index 4 \
    --index $i --threshold -10"
done